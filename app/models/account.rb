...
class Account < ApplicationRecord
  ...
  validates :display_name, length: { maximum: 500 }, if: -> { local? && will_save_change_to_display_name? }
  validates :note, note_length: { maximum: 4000 }, if: -> { local? && will_save_change_to_note? }
