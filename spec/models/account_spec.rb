...
RSpec.describe Account, type: :model do
  ...
  describe 'validations' do
    ...
    context 'when is local' do
      ...
      it 'is invalid if the display name is longer than 500 characters' do
        account = Fabricate.build(:account, display_name: Faker::Lorem.characters(number: 501))
        account.valid?
        expect(account).to model_have_error_on_field(:display_name)
      end

      it 'is invalid if the note is longer than 4000 characters' do
        account = Fabricate.build(:account, note: Faker::Lorem.characters(number: 4001))
        account.valid?
    ...
    context 'when is remote' do
      ...
      it 'is valid even if the display name is longer than 500 characters' do
        account = Fabricate.build(:account, domain: 'domain', display_name: Faker::Lorem.characters(number: 501))
        account.valid?
        expect(account).not_to model_have_error_on_field(:display_name)
      end

      it 'is valid even if the note is longer than 4000 characters' do
        account = Fabricate.build(:account, domain: 'domain', note: Faker::Lorem.characters(number: 4001))
        account.valid?
